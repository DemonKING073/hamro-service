...
type CustomDrawerProps = DrawerProps & {
    ...
    visible: boolean;
    onClose: ()=> void;
    data: undefined | ProductProps |null;
...
const { Option } = Select;
const UpdateProductForms:FC<CustomDrawerProps> = ({onFinish, visible, onClose, data, region}) => {
    ...
    return(
        <Drawer visible={visible} onClose={onClose}  closeIcon={<CloseOutlined style={{color:'white'}} />} headerStyle={{backgroundColor:'var(--primary)'}} title={<span style={{color:'white'}}>Update Product</span>} placement='right'>
            <Form form={form} layout='vertical' onFinish={showValues} initialValues={{
                regionId: region,
                baseProduct: data?.baseProductId,
